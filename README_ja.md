...
テストは `test/js/` にノードごとに配置しています（コピペ復元は `paste_restore.test.js` も参照）。純粋ロジックは `js/logic/` にあり、読込み・コピペ・型同期の回帰を検証します。

コピペやワークフロー読込直後は、リンク復元が完了するまで動的スロットの削除を一時的に抑え、その後 `stabilize()` で未使用スロットを整理します。
