...
import {
    buildSwitchOutputValues,
    computeOutputLayout,
} from "../../js/logic/any-output-switch-layout.js";
...
describe("コピー＆ペースト復元（接続維持）", () => {
    ...
    describe("Split Primitives (misc)", () => {
        it("復元中・Combine未確定・combined接続ありではキャッシュ出力構成を維持", () => {
            ...
            expect(
                shouldHoldSplitOutputsWithoutCombine(true, null, true, cached),
            ).toBe(true);
        });
    ...
    describe("Any Switch (misc)", () => {
        ...
        it("select_index 末尾移動後も入力リンクの target_slot が有効", () => {
            ...
            expect(
                verifyInputLinksAfterSelectIndexMove(inputs, linksToNode, nodeId, 0, 2),
            ).toBe(true);
        ...
        it("ペースト後グラフで any_* / select_index リンクが解決できる", () => {
            ...
            const outputs = [{ name: "*", links: [20] }, { name: "index", links: [21] }];
            const links = {
